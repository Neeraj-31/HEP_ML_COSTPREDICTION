...
def make_search_fn():
    def _fn(X_tr_s, y_tr_log):
        search = GridSearchCV(
            XGBRegressor(random_state=C.RANDOM_SEED, verbosity=0,
                         objective="reg:squarederror", n_jobs=1),
            XGB_GRID, cv=5, scoring="neg_mean_absolute_error", n_jobs=-1, refit=True
        )
...
xgb_75, sc_75, pred_25, m = C.run_split(
...
xgb_65, sc_65, pred_35, m = C.run_split(
...
# ── 5. Summary ────────────────────────────────────────────────────────────
df_res = C.summarize(results, "XGBoost (all splits)")

# ── 6. Feature importances (65/35 model) ──────────────────────────────────
importances = pd.Series(xgb_65.feature_importances_, index=features).sort_values(
    ascending=False)
print("\nFeature importances (XGBoost 65/35):")
print(importances.to_string())

# ── 7. Plots ───────────────────────────────────────────────────────────
C.plot_predictions(y_te25, pred_25, y_te35, pred_35, y_full, y_loo_pred,
                    MODEL_TAG, "dam_xgb_predictions.png")
C.plot_importances(importances, MODEL_TAG, "dam_xgb_feature_importance.png",
                    ylabel="Feature Importance (gain)")

# ── 8. Save model ────────────────────────────────────────────────────────
C.save_model({"model": xgb_65, "scaler": sc_65, "y_shift": y_shift,
              "features": features}, "dam_best_xgb.pkl")

best_row = df_res.iloc[0]
print(f"\nBest XGB configuration overall: {best_row['label']}  "
