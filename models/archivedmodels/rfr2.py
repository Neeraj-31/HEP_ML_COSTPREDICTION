...
# -- Log-transform target (shift so all values > 0 before log) ---------------
y_shift = y_full.min() - 1.0          # guarantees y_shifted > 0
y_log   = np.log(y_full - y_shift)    # train in log space
...
def run_split(X_train, X_test, y_train_log, y_test_orig, split_label):
    ...
    rf_cv.fit(X_tr_s, y_train_log)
    best_rf = rf_cv.best_estimator_
    print(f"\n[RF] Best params: {rf_cv.best_params_}")

    rf_pred_log  = best_rf.predict(X_te_s)
    rf_pred_orig = inv_log(rf_pred_log)
    r_rf = evaluate(y_test_orig, rf_pred_orig, label=f"Random Forest ({split_label})")
    results.append(r_rf)

    return best_rf, scaler, rf_pred_orig

# ── 5.  75/25 split ──────────────────────────────────────────────────────────
X_tr75, X_te25, yl_tr75, yl_te25, y_tr75, y_te25 = train_test_split(
    X_full, y_log, y_full, test_size=0.25, random_state=RANDOM_SEED
)
rf_75, sc_75, rf_pred_25 = run_split(X_tr75, X_te25, yl_tr75, y_te25, "75/25")

# ── 6.  65/35 split ──────────────────────────────────────────────────────────
X_tr65, X_te35, yl_tr65, yl_te35, y_tr65, y_te35 = train_test_split(
    X_full, y_log, y_full, test_size=0.35, random_state=RANDOM_SEED
)
rf_65, sc_65, rf_pred_35 = run_split(X_tr65, X_te35, yl_tr65, y_te35, "65/35")
...
for train_idx, test_idx in loo.split(X_full):
    ...
    params = rf_65.get_params()
    m_clone = RandomForestRegressor(**params)
    m_clone.fit(X_tr_s, y_log[train_idx])
    y_loo_pred[test_idx] = inv_log(m_clone.predict(X_te_s))

r = evaluate(y_full, y_loo_pred, label="Random Forest (LOO)")
results.append(r)
...
importances = pd.Series(rf_65.feature_importances_, index=ALL_FEATURES).sort_values(ascending=False)
print("\nTop 15 feature importances (Random Forest 65/35):")
...
scatter(axes[2], y_full, y_loo_pred,  "RF — LOO")
plt.tight_layout()
...
with open("hep_best_rf.pkl", "wb") as f:
    pickle.dump({"model": rf_65, "scaler": sc_65, "y_shift": y_shift,
