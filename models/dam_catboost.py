...
def make_search_fn():
    def _fn(X_tr_s, y_tr_log):
        search = GridSearchCV(
            CatBoostRegressor(random_state=C.RANDOM_SEED, verbose=0,
                              loss_function="MAE", thread_count=1),
            CB_GRID, cv=5, scoring="neg_mean_absolute_error", n_jobs=-1, refit=True
        )
        search.fit(X_tr_s, y_tr_log)
        print(f"[CatBoost] Best params: {search.best_params_}")
...
cb_75, sc_75, pred_25, m = C.run_split(
...
cb_65, sc_65, pred_35, m = C.run_split(
...
# ── 5. Summary ────────────────────────────────────────────────────────────
df_res = C.summarize(results, "CatBoost (all splits)")

# ── 6. Feature importances (65/35 model) ──────────────────────────────────
importances = pd.Series(cb_65.get_feature_importance(), index=features).sort_values(
    ascending=False)
print("\nFeature importances (CatBoost 65/35):")
print(importances.to_string())

# ── 7. Plots ───────────────────────────────────────────────────────────
C.plot_predictions(y_te25, pred_25, y_te35, pred_35, y_full, y_loo_pred,
                    MODEL_TAG, "dam_catboost_predictions.png")
C.plot_importances(importances, MODEL_TAG, "dam_catboost_feature_importance.png",
                    ylabel="Feature Importance")

# ── 8. Save model ────────────────────────────────────────────────────────
C.save_model({"model": cb_65, "scaler": sc_65, "y_shift": y_shift,
              "features": features}, "dam_best_catboost.pkl")

best_row = df_res.iloc[0]
print(f"\nBest CatBoost configuration overall: {best_row['label']}  "
